...
vector<Point> read_file()
{
	vector<Point> p;
	ifstream ifs;
	ifs.open("location_old1.txt", ios::in);
	if (!ifs.is_open())
	{
		cout << "不能打开文件" << endl;
		system("pause");
...
{
	ofstream ofs;
	ofs.open("station_918_connection.txt", ios::out);
	int i = 0;
	for (vector<Point>::iterator it = v.begin(); it != v.end(); it++)
	{
		//ofs << q[i] << " " << fixed << setprecision(4) << it->x << " " << fixed << setprecision(4) << it->y << endl;
		ofs << it->origin_id << " " <<q[i] << endl;
		i++;
